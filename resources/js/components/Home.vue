<template>
    <v-row>
        <v-col class="col-12" sm="6" md="3">
            <v-card to="/user" class="mx-auto" max-width="344" color="pink lighten-2" dark>
                <v-list-item three-line>
                    <v-list-item-content>
                        <v-list-item-title class="headline mb-1"
                            >User</v-list-item-title
                        >
                        <v-list-item-subtitle>20 users</v-list-item-subtitle>
                    </v-list-item-content>

                    <v-list-item-avatar tile size="80">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="698"
                            height="698"
                            viewBox="0 0 698 698"
                        >
                            <title>undraw_profile_pic_ic5t</title>
                            <g>
                                <defs>
                                    <linearGradient
                                        id="b247946c-c62f-4d08-994a-4c3d64e1e98f"
                                        x1="349"
                                        y1="698"
                                        x2="349"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop
                                            offset="0"
                                            stop-color="gray"
                                            stop-opacity=".25"
                                        ></stop>
                                        <stop
                                            offset=".54"
                                            stop-color="gray"
                                            stop-opacity=".12"
                                        ></stop>
                                        <stop
                                            offset="1"
                                            stop-color="gray"
                                            stop-opacity=".1"
                                        ></stop>
                                    </linearGradient>
                                </defs>
                                <circle
                                    cx="349"
                                    cy="349"
                                    r="349"
                                    fill="url(#b247946c-c62f-4d08-994a-4c3d64e1e98f)"
                                    opacity=".5"
                                ></circle>
                                <circle
                                    cx="349.68"
                                    cy="346.77"
                                    r="341.64"
                                    fill="#f5f5f5"
                                ></circle>
                                <path
                                    d="M601 790.76a340 340 0 0 0 187.79-56.2c-12.59-68.8-60.5-72.72-60.5-72.72h-264.2s-45.21 3.71-59.33 67A340.07 340.07 0 0 0 601 790.76z"
                                    transform="translate(-251 -101)"
                                    fill="#854fee"
                                ></path>
                                <circle
                                    cx="346.37"
                                    cy="339.57"
                                    r="164.9"
                                    fill="#333"
                                ></circle>
                                <path
                                    d="M293.15 476.92h105.66v84.53A52.83 52.83 0 0 1 346 614.28a52.83 52.83 0 0 1-52.83-52.83v-84.53z"
                                    opacity=".1"
                                ></path>
                                <path
                                    d="M296.5 473h99a3.35 3.35 0 0 1 3.35 3.35v81.18A52.83 52.83 0 0 1 346 610.37a52.83 52.83 0 0 1-52.83-52.83v-81.19a3.35 3.35 0 0 1 3.33-3.35z"
                                    fill="#fdb797"
                                ></path>
                                <path
                                    d="M544.34 617.82a152.07 152.07 0 0 0 105.66.29v-13H544.34z"
                                    transform="translate(-251 -101)"
                                    opacity=".1"
                                ></path>
                                <circle
                                    cx="346.37"
                                    cy="372.44"
                                    r="151.45"
                                    fill="#fdb797"
                                ></circle>
                                <path
                                    d="M489.49 335.68S553.32 465.24 733.37 390l-41.92-65.73-74.31-26.67z"
                                    transform="translate(-251 -101)"
                                    opacity=".1"
                                ></path>
                                <path
                                    d="M489.49 333.78s63.83 129.56 243.88 54.3l-41.92-65.73-74.31-26.67z"
                                    transform="translate(-251 -101)"
                                    fill="#333"
                                ></path>
                                <path
                                    d="M488.93 325a87.49 87.49 0 0 1 21.69-35.27c29.79-29.45 78.63-35.66 103.68-69.24 6 9.32 1.36 23.65-9 27.65 24-.16 51.81-2.26 65.38-22a44.89 44.89 0 0 1-7.57 47.4c21.27 1 44 15.4 45.34 36.65.92 14.16-8 27.56-19.59 35.68s-25.71 11.85-39.56 14.9c-40.44 8.93-186.76 46.3-160.37-35.77z"
                                    transform="translate(-251 -101)"
                                    fill="#333"
                                ></path>
                                <ellipse
                                    cx="194.86"
                                    cy="372.3"
                                    rx="14.09"
                                    ry="26.42"
                                    fill="#fdb797"
                                ></ellipse>
                                <ellipse
                                    cx="497.8"
                                    cy="372.3"
                                    rx="14.09"
                                    ry="26.42"
                                    fill="#fdb797"
                                ></ellipse>
                            </g>
                        </svg>
                    </v-list-item-avatar>
                </v-list-item>
            </v-card>
        </v-col>

        <v-col class="col-12" sm="6" md="3">
            <v-card to="inventory" class="mx-auto" max-width="344" color="blue lighten-2" dark>
                <v-list-item three-line>
                    <v-list-item-content>
                        <v-list-item-title class="headline mb-1"
                            >Inventory</v-list-item-title
                        >
                        <v-list-item-subtitle>20 Item</v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-avatar tile size="80">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="814"
                            height="778"
                            viewBox="0 0 814 778.22"
                        >
                            <title>undraw_files1_9ool</title>
                            <g>
                                <defs>
                                    <linearGradient
                                        id="29b91a8c-5031-4728-9934-4de601869dbd"
                                        x1="253.26"
                                        y1="778.22"
                                        x2="253.26"
                                        y2="105.1"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop
                                            offset=".01"
                                            stop-color="gray"
                                            stop-opacity=".25"
                                        ></stop>
                                        <stop
                                            offset=".54"
                                            stop-color="gray"
                                            stop-opacity=".12"
                                        ></stop>
                                        <stop
                                            offset="1"
                                            stop-color="gray"
                                            stop-opacity=".1"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="1256144c-81fa-4390-be21-e94fb20836e2"
                                        x1="49.76"
                                        y1="202.38"
                                        x2="49.76"
                                        y2="112.93"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop
                                            offset="0"
                                            stop-color="#b3b3b3"
                                            stop-opacity=".25"
                                        ></stop>
                                        <stop
                                            offset=".54"
                                            stop-color="#b3b3b3"
                                            stop-opacity=".1"
                                        ></stop>
                                        <stop
                                            offset="1"
                                            stop-color="#b3b3b3"
                                            stop-opacity=".05"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="8f0ddb95-6166-4bb1-9b4d-46cd9e3d30e6"
                                        x1="408.68"
                                        y1="722.31"
                                        x2="408.68"
                                        y2="49.2"
                                        xmlns:xlink="http://www.w3.org/1999/xlink"
                                        xlink:href="#29b91a8c-5031-4728-9934-4de601869dbd"
                                    ></linearGradient>
                                    <linearGradient
                                        id="2acd927f-116a-42d0-926b-2e100ad401a0"
                                        x1="205.18"
                                        y1="146.48"
                                        x2="205.18"
                                        y2="57.02"
                                        xmlns:xlink="http://www.w3.org/1999/xlink"
                                        xlink:href="#1256144c-81fa-4390-be21-e94fb20836e2"
                                    ></linearGradient>
                                    <linearGradient
                                        id="dd98f1d5-faf2-451f-bb12-7f13808eec29"
                                        x1="560.74"
                                        y1="673.12"
                                        x2="560.74"
                                        y2="0"
                                        xmlns:xlink="http://www.w3.org/1999/xlink"
                                        xlink:href="#29b91a8c-5031-4728-9934-4de601869dbd"
                                    ></linearGradient>
                                    <linearGradient
                                        id="523444de-12f9-4e88-a28a-f69f2aefc1da"
                                        x1="357.24"
                                        y1="97.28"
                                        x2="357.24"
                                        y2="7.83"
                                        xmlns:xlink="http://www.w3.org/1999/xlink"
                                        xlink:href="#1256144c-81fa-4390-be21-e94fb20836e2"
                                    ></linearGradient>
                                </defs>
                                <path
                                    fill="url(#29b91a8c-5031-4728-9934-4de601869dbd)"
                                    d="M506.51 778.22H0V189.97l76.53-84.87h429.98v673.12z"
                                ></path>
                                <path
                                    fill="#fafafa"
                                    d="M498.69 760.33H8.95V194.56l73.99-81.63h415.75v647.4z"
                                ></path>
                                <path
                                    fill="#e0e0e0"
                                    d="M56.76 182.81h238.59v40.25H56.76z"
                                ></path>
                                <path
                                    fill="#e0e0e0"
                                    d="M57.24 292.95h174.84v40.25H57.24z"
                                ></path>
                                <path
                                    fill="#6c63ff"
                                    d="M57.24 458.43h392.18v147.59H57.24z"
                                ></path>
                                <path
                                    fill="#f5f5f5"
                                    d="M360.55 183.37h88.87v40.25h-88.87z"
                                ></path>
                                <path
                                    fill="#f5f5f5"
                                    d="M360.55 245.99h88.87v40.25h-88.87z"
                                ></path>
                                <path
                                    fill="url(#1256144c-81fa-4390-be21-e94fb20836e2)"
                                    d="M90.57 112.93v89.45H8.95v-7.82l73.79-81.63h7.83z"
                                ></path>
                                <path
                                    fill="#eee"
                                    d="M8.75 194.56l73.99-81.63v81.63H8.75z"
                                ></path>
                                <path
                                    fill="url(#8f0ddb95-6166-4bb1-9b4d-46cd9e3d30e6)"
                                    d="M661.93 722.31H155.42V134.06l76.53-84.86h429.98v673.11z"
                                ></path>
                                <path
                                    fill="#fafafa"
                                    d="M654.11 704.42H164.37V138.65l73.99-81.63h415.75v647.4z"
                                ></path>
                                <path
                                    fill="#e0e0e0"
                                    d="M212.18 126.91h238.59v40.25H212.18z"
                                ></path>
                                <path
                                    fill="#e0e0e0"
                                    d="M212.66 237.04H387.5v40.25H212.66z"
                                ></path>
                                <path
                                    fill="#6c63ff"
                                    d="M212.66 402.53h392.18v147.59H212.66z"
                                ></path>
                                <path
                                    fill="#f5f5f5"
                                    d="M515.97 127.47h88.87v40.25h-88.87z"
                                ></path>
                                <path
                                    fill="#f5f5f5"
                                    d="M515.97 190.08h88.87v40.25h-88.87z"
                                ></path>
                                <path
                                    fill="url(#2acd927f-116a-42d0-926b-2e100ad401a0)"
                                    d="M245.99 57.02v89.45h-81.62v-7.82l73.79-81.63h7.83z"
                                ></path>
                                <path
                                    fill="#eee"
                                    d="M164.17 138.65l73.99-81.63v81.63h-73.99z"
                                ></path>
                                <path
                                    fill="url(#dd98f1d5-faf2-451f-bb12-7f13808eec29)"
                                    d="M814 673.12H307.49V84.87L384.01 0H814v673.12z"
                                ></path>
                                <path
                                    fill="#fff"
                                    d="M806.17 655.23H316.43V89.45l74-81.62h415.74v647.4z"
                                ></path>
                                <path
                                    fill="#e0e0e0"
                                    d="M364.25 77.71h238.59v40.25H364.25z"
                                ></path>
                                <path
                                    fill="#e0e0e0"
                                    d="M364.73 187.85h174.84v40.25H364.73z"
                                ></path>
                                <path
                                    fill="#6c63ff"
                                    d="M364.73 353.33h392.18v147.59H364.73z"
                                ></path>
                                <path
                                    fill="#e0e0e0"
                                    d="M668.04 78.27h88.87v40.25h-88.87z"
                                ></path>
                                <path
                                    fill="#e0e0e0"
                                    d="M668.04 140.88h88.87v40.25h-88.87z"
                                ></path>
                                <path
                                    fill="#e0e0e0"
                                    d="M668.04 203.5h88.87v40.25h-88.87z"
                                ></path>
                                <path
                                    fill="url(#523444de-12f9-4e88-a28a-f69f2aefc1da)"
                                    d="M398.06 7.83v89.45h-81.63v-7.83l73.8-81.62h7.83z"
                                ></path>
                                <path
                                    fill="#eee"
                                    d="M316.23 89.45l74-81.62v81.62h-74z"
                                ></path>
                            </g>
                        </svg>
                    </v-list-item-avatar>
                </v-list-item>
            </v-card>
        </v-col>

        <v-col class="col-12" sm="6" md="3">
            <v-card to="/buy" class="mx-auto" max-width="344" color="teal lighten-2" dark>
                <v-list-item three-line>
                    <v-list-item-content>
                        <v-list-item-title class="headline mb-1"
                            >Buy</v-list-item-title
                        >
                        <v-list-item-subtitle>20 users</v-list-item-subtitle>
                    </v-list-item-content>

                    <v-list-item-avatar tile size="80">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="896"
                            height="747"
                            viewBox="0 0 896 747.971"
                        >
                            <title>undraw_empty_cart_co35</title>
                            <g>
                                <path
                                    d="M193.634 788.752c12.428 23.05 38.806 32.944 38.806 32.944s6.227-27.476-6.201-50.525-38.806-32.943-38.806-32.943-6.227 27.475 6.201 50.524z"
                                    transform="translate(-152 -76.014)"
                                    fill="#2f2e41"
                                ></path>
                                <path
                                    d="M202.177 781.17c22.438 13.499 31.08 40.313 31.08 40.313s-27.738 4.927-50.177-8.573S152 772.596 152 772.596s27.738-4.926 50.177 8.573z"
                                    transform="translate(-152 -76.014)"
                                    fill="#3f51b5"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M413.248 35.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M513.249 37.408h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M452.248 37.408h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M484.248 131.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M522.249 113.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M583.249 113.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M670.249 176.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M708.249 158.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M769.249 158.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M656.249 640.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M694.249 622.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M755.249 622.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M417.248 319.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M455.248 301.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M516.249 301.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M461.248 560.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M499.248 542.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M560.249 542.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M685.249 487.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M723.249 469.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M784.249 469.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#2f2e41"
                                    d="M362.06 702.184H125.274v-1.703h235.082v-82.62H145.18l-10.453-21.777 1.536-.737 9.989 20.81H362.06v86.027z"
                                ></path>
                                <circle
                                    cx="156.789"
                                    cy="726.033"
                                    r="17.887"
                                    fill="#3f3d56"
                                ></circle>
                                <circle
                                    cx="333.101"
                                    cy="726.033"
                                    r="17.887"
                                    fill="#3f3d56"
                                ></circle>
                                <circle
                                    cx="540.927"
                                    cy="346.153"
                                    r="11.073"
                                    fill="#3f3d56"
                                ></circle>
                                <path
                                    d="M539.385 665.767H273.237l-57.589-188.236h383.045l-.349 1.108zm-264.888-1.703h263.639l58.234-184.83H217.95z"
                                    transform="translate(-152 -76.014)"
                                    fill="#2f2e41"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M366.61 579.958H132.842L82.26 413.015h336.441l-.306.983-51.785 165.96z"
                                ></path>
                                <path
                                    fill="#2f2e41"
                                    d="M451.465 384.7l-1.647-.437 11.241-42.369h65.389v1.704H462.37L451.465 384.7z"
                                ></path>
                                <path
                                    fill="#2f2e41"
                                    d="M82.258 458.584h345.293v1.704H82.258z"
                                ></path>
                                <path
                                    fill="#2f2e41"
                                    d="M101.459 521.344h306.319v1.704H101.459z"
                                ></path>
                                <path
                                    fill="#2f2e41"
                                    d="M254.314 402.368h1.704v186.533h-1.704z"
                                ></path>
                                <path
                                    transform="rotate(-86.249 362.441 614.788)"
                                    fill="#2f2e41"
                                    d="M385.557 570.797h186.929v1.704H385.557z"
                                ></path>
                                <path
                                    transform="rotate(-3.729 -908.232 2868.285)"
                                    fill="#2f2e41"
                                    d="M334.457 478.185h1.704v186.929h-1.704z"
                                ></path>
                                <path fill="#2f2e41" d="M0 745h896v2H0z"></path>
                                <path
                                    d="M747.41 137.89s14.62 41.607 5.623 48.008 30.361 58.675 30.361 58.675l47.23-12.802-25.864-43.74s-3.374-43.74-3.374-50.14-53.975 0-53.975 0z"
                                    transform="translate(-152 -76.014)"
                                    fill="#a0616a"
                                ></path>
                                <path
                                    d="M747.41 137.89s14.62 41.607 5.623 48.008 30.361 58.675 30.361 58.675l47.23-12.802-25.864-43.74s-3.374-43.74-3.374-50.14-53.975 0-53.975 0z"
                                    transform="translate(-152 -76.014)"
                                    opacity=".1"
                                ></path>
                                <path
                                    d="M722.874 434.468s-4.268 53.342 0 81.08 10.668 104.548 10.668 104.548 0 145.089 23.47 147.222 40.54 4.268 42.673-4.267-10.668-12.802-4.267-17.07 8.535-19.202 0-36.271 0-189.895 0-189.895l40.54 108.816s4.267 89.613 8.534 102.415-4.267 36.272 10.668 38.406 32.005-10.668 40.54-14.935-12.802-4.268-8.535-6.401 17.07-8.535 12.802-10.669-8.535-104.549-8.535-104.549-11.735-218.7-26.67-227.234-24.537 6.166-24.537 6.166z"
                                    transform="translate(-152 -76.014)"
                                    fill="#2f2e41"
                                ></path>
                                <path
                                    d="M761.28 758.784v17.07s-19.203 46.399 0 46.399 34.138 4.808 34.138-1.593V763.05z"
                                    transform="translate(-152 -76.014)"
                                    fill="#2f2e41"
                                ></path>
                                <path
                                    d="M887.165 758.754v17.069s19.203 46.4 0 46.4-34.138 4.808-34.138-1.593v-57.61z"
                                    transform="translate(-152 -76.014)"
                                    fill="#2f2e41"
                                ></path>
                                <circle
                                    cx="625.282"
                                    cy="54.408"
                                    r="38.406"
                                    fill="#a0616a"
                                ></circle>
                                <path
                                    d="M765.547 201.9s10.668 32.005 27.737 25.604l17.07-6.401 29.87 204.83s-23.47 34.14-57.608 12.803-17.07-236.836-17.07-236.836z"
                                    transform="translate(-152 -76.014)"
                                    fill="#3f51b5"
                                ></path>
                                <path
                                    d="M795.418 195.499l9.601-20.27s56.542 26.671 65.077 35.206 8.534 21.336 8.534 21.336l-14.935 53.342s4.267 117.35 4.267 121.618 14.936 27.737 4.267 19.203-12.801-17.07-21.336-4.268-27.738 27.738-27.738 27.738z"
                                    transform="translate(-152 -76.014)"
                                    fill="#3f3d56"
                                ></path>
                                <path
                                    d="M870.096 349.122l-6.401 59.742s-38.406 34.139-29.871 36.273 12.802-6.401 12.802-6.401 14.935 14.935 23.47 6.4 29.871-89.613 29.871-89.613z"
                                    transform="translate(-152 -76.014)"
                                    fill="#a0616a"
                                ></path>
                                <path
                                    d="M778.1 76.144c-8.514-.304-17.625-.455-24.804 4.133a36.313 36.313 0 0 0-8.572 8.392c-6.992 8.838-13.033 19.96-10.436 30.925l3.016-1.176a19.75 19.75 0 0 1-1.905 8.462c.425-1.235 1.848.762 1.467 2.011l-3.323 10.901c5.462-2.002 12.257 2.052 13.088 7.81.38-12.662 1.693-27.18 11.964-34.594 5.18-3.738 11.735-4.88 18.042-5.893 5.818-.935 11.918-1.827 17.49.089s10.32 7.615 9.056 13.37c2.57-.885 5.443.906 6.713 3.31s1.337 5.237 1.375 7.954c2.74 1.936 5.857-1.908 6.973-5.07 2.62-7.425 4.95-15.328 3.538-23.074s-7.724-15.148-15.597-15.174a5.467 5.467 0 0 0 1.422-3.849l-6.49-.548a7.172 7.172 0 0 0 4.287-2.26c-2.606 2.868-23.09-5.568-27.304-5.719z"
                                    transform="translate(-152 -76.014)"
                                    fill="#2f2e41"
                                ></path>
                                <path
                                    d="M776.215 189.098s-17.37-17.02-23.62-15.978-14.786 15.978-14.786 15.978-51.207 17.07-49.074 34.138 25.604 100.282 25.604 100.282 19.203 100.282 2.134 110.95 81.079 38.406 83.212 25.604 6.401-140.821 0-160.024-23.47-110.95-23.47-110.95z"
                                    transform="translate(-152 -76.014)"
                                    fill="#3f3d56"
                                ></path>
                                <path
                                    d="M850.893 223.236h26.383s18.424 81.08 20.557 89.614 6.401 49.074 4.268 49.074-44.807-8.535-44.807-2.134z"
                                    transform="translate(-152 -76.014)"
                                    fill="#3f3d56"
                                ></path>
                                <path
                                    d="M850 424.014H749c-9.856-45.34-10.68-89.146 0-131h101c-16.3 41.101-17.318 84.607 0 131z"
                                    transform="translate(-152 -76.014)"
                                    fill="#f2f2f2"
                                ></path>
                                <path
                                    d="M707.938 368.325l29.871 12.802s57.609 8.535 57.609-14.936-57.609-10.668-57.609-10.668l-19.204-6.14z"
                                    transform="translate(-152 -76.014)"
                                    fill="#a0616a"
                                ></path>
                                <path
                                    d="M714.339 210.435l-25.604 6.4-19.203 113.084s-6.4 29.871 4.268 32.005 40.539 19.203 40.539 19.203 4.267-32.005 12.802-32.005l-21.337-17.07 12.802-74.677z"
                                    transform="translate(-152 -76.014)"
                                    fill="#3f3d56"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M60.248 352.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M98.249 334.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M159.249 334.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M109.249 56.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M209.249 58.408h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M148.249 58.408h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M250.249 253.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M350.248 255.408h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M289.248 255.408h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M12.248 252.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M112.249 254.408h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M51.248 254.408h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M180.249 152.908h140v2h-140z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M218.249 134.908h2v18.5h-2z"
                                ></path>
                                <path
                                    fill="#f2f2f2"
                                    d="M279.248 134.908h2v18.5h-2z"
                                ></path>
                            </g>
                        </svg>
                    </v-list-item-avatar>
                </v-list-item>
            </v-card>
        </v-col>

        <v-col class="col-12" sm="6" md="3">
            <v-card to="/location"
                class="mx-auto"
                max-width="344"
                color="indigo lighten-2"
                dark
            >
                <v-list-item three-line>
                    <v-list-item-content>
                        <v-list-item-title class="headline mb-1"
                            >Location</v-list-item-title
                        >
                        <v-list-item-subtitle
                            >20 locations</v-list-item-subtitle
                        >
                    </v-list-item-content>

                    <v-list-item-avatar tile size="80">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width="887"
                            height="794"
                            viewBox="0 0 887.591 794.699"
                        >
                            <title>undraw_Map_light_3hjy</title>
                            <g>
                                <defs>
                                    <linearGradient
                                        id="e7432531-051f-4229-a0a7-c46f90e1c00c"
                                        x1="148.905"
                                        y1="794.699"
                                        x2="148.905"
                                        y2="65.634"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop
                                            offset="0"
                                            stop-color="gray"
                                            stop-opacity=".25"
                                        ></stop>
                                        <stop
                                            offset=".535"
                                            stop-color="gray"
                                            stop-opacity=".12"
                                        ></stop>
                                        <stop
                                            offset="1"
                                            stop-color="gray"
                                            stop-opacity=".1"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="b47202c6-c1c9-40b8-92af-db2c55bf8bd1"
                                        x1="446.716"
                                        y1="794.699"
                                        x2="446.716"
                                        y2="65.634"
                                        xmlns:xlink="http://www.w3.org/1999/xlink"
                                        xlink:href="#e7432531-051f-4229-a0a7-c46f90e1c00c"
                                    ></linearGradient>
                                    <linearGradient
                                        id="adbec034-83c8-4647-bb3b-2a01b75562b1"
                                        x1="741.606"
                                        y1="794.699"
                                        x2="741.606"
                                        y2="65.634"
                                        xmlns:xlink="http://www.w3.org/1999/xlink"
                                        xlink:href="#e7432531-051f-4229-a0a7-c46f90e1c00c"
                                    ></linearGradient>
                                    <linearGradient
                                        id="eb6a4cdb-41e1-4fe3-bddb-eac7508f621d"
                                        x1="889.9"
                                        y1="291.937"
                                        x2="889.9"
                                        y2="71.206"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop
                                            offset="0"
                                            stop-color="#b3b3b3"
                                            stop-opacity=".25"
                                        ></stop>
                                        <stop
                                            offset=".535"
                                            stop-color="#b3b3b3"
                                            stop-opacity=".1"
                                        ></stop>
                                        <stop
                                            offset="1"
                                            stop-color="#b3b3b3"
                                            stop-opacity=".05"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="f829b744-a02a-4a2c-9fed-3f5da7d4212f"
                                        x1="733.695"
                                        y1="105.86"
                                        x2="733.695"
                                        y2="63.032"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop
                                            offset="0"
                                            stop-opacity=".12"
                                        ></stop>
                                        <stop
                                            offset=".551"
                                            stop-opacity=".09"
                                        ></stop>
                                        <stop
                                            offset="1"
                                            stop-opacity=".02"
                                        ></stop>
                                    </linearGradient>
                                    <linearGradient
                                        id="ae45afba-725c-45f7-9e29-49ac6fec796e"
                                        x1="311.146"
                                        y1="736.281"
                                        x2="311.146"
                                        y2="513.99"
                                        xmlns:xlink="http://www.w3.org/1999/xlink"
                                        xlink:href="#eb6a4cdb-41e1-4fe3-bddb-eac7508f621d"
                                    ></linearGradient>
                                    <linearGradient
                                        id="f4c06303-04ff-41d3-ad1f-a3b84d982f8e"
                                        x1="154.941"
                                        y1="549.261"
                                        x2="154.941"
                                        y2="506.13"
                                        gradientUnits="userSpaceOnUse"
                                    >
                                        <stop
                                            offset=".005"
                                            stop-opacity=".12"
                                        ></stop>
                                        <stop
                                            offset=".551"
                                            stop-opacity=".09"
                                        ></stop>
                                        <stop
                                            offset="1"
                                            stop-opacity=".05"
                                        ></stop>
                                    </linearGradient>
                                </defs>
                                <path
                                    fill="url(#e7432531-051f-4229-a0a7-c46f90e1c00c)"
                                    d="M297.81 766.722L0 794.699V93.611l297.81-27.977v701.088z"
                                ></path>
                                <path
                                    fill="url(#b47202c6-c1c9-40b8-92af-db2c55bf8bd1)"
                                    d="M297.81 766.722l297.811 27.977V93.611L297.81 65.634v701.088z"
                                ></path>
                                <path
                                    fill="url(#adbec034-83c8-4647-bb3b-2a01b75562b1)"
                                    d="M887.591 766.722l-291.97 27.977V93.611l291.97-27.977v701.088z"
                                ></path>
                                <path
                                    fill="#f5f5f5"
                                    d="M298.865 747.261L9.488 774.143V100.511l289.377-26.882v673.632z"
                                ></path>
                                <path
                                    fill="#fff"
                                    d="M298.865 747.261l289.377 26.882V100.511L298.865 73.629v673.632z"
                                ></path>
                                <path
                                    fill="#f5f5f5"
                                    d="M877.618 747.261l-289.376 26.882V100.511l289.376-26.882v673.632z"
                                ></path>
                                <path
                                    fill="#3f51b5"
                                    opacity=".3"
                                    d="M298.865 135.494l-219.8 7.712v101.203h186.592v-36.592l33.208-1.165 50.601 15.619v-71.158l-50.601-15.619z"
                                ></path>
                                <path
                                    fill="#3f51b5"
                                    opacity=".3"
                                    d="M298.865 701.403l-246.682 23.72v-90.134l246.682-23.72v90.134z"
                                ></path>
                                <path
                                    fill="#3f51b5"
                                    opacity=".3"
                                    d="M271.865 422.403l-246.682 23.72v-90.134l246.682-23.72v90.134z"
                                ></path>
                                <path
                                    fill="#3f51b5"
                                    opacity=".3"
                                    d="M444.344 631.826h107.528v82.227H444.344z"
                                ></path>
                                <path
                                    fill="#3f51b5"
                                    opacity=".3"
                                    d="M409.344 438.826h107.528v82.227H409.344z"
                                ></path>
                                <path
                                    fill="#3f51b5"
                                    opacity=".3"
                                    d="M453.344 181.826h107.528v82.227H453.344z"
                                ></path>
                                <path
                                    fill="#3f51b5"
                                    opacity=".3"
                                    d="M836.505 676.102l-167.617 14.232V581.225l167.617-14.232v109.109z"
                                ></path>
                                <path
                                    fill="#3f51b5"
                                    opacity=".3"
                                    d="M816.505 292.102l-167.617 14.232V197.225l167.617-14.232v109.109z"
                                ></path>
                                <path
                                    d="M958.282 142.038c0 39.12-68.382 149.9-68.382 149.9s-68.382-110.78-68.382-149.9 30.616-70.832 68.382-70.832 68.382 31.713 68.382 70.832z"
                                    transform="translate(-156.205 -52.65)"
                                    fill="url(#eb6a4cdb-41e1-4fe3-bddb-eac7508f621d)"
                                ></path>
                                <ellipse
                                    cx="733.695"
                                    cy="84.446"
                                    rx="20.674"
                                    ry="21.414"
                                    fill="url(#f829b744-a02a-4a2c-9fed-3f5da7d4212f)"
                                ></ellipse>
                                <path
                                    d="M953.177 142.165c0 34.947-63.277 133.912-63.277 133.912s-63.277-98.965-63.277-133.912a63.277 63.277 0 1 1 126.554 0z"
                                    transform="translate(-156.205 -52.65)"
                                    fill="#3f51b5"
                                ></path>
                                <circle
                                    cx="733.695"
                                    cy="85.1"
                                    r="19.13"
                                    fill="#fff"
                                ></circle>
                                <path
                                    d="M381.558 585.322c0 39.396-70.412 150.959-70.412 150.959s-70.411-111.563-70.411-150.959 31.524-71.332 70.411-71.332 70.412 31.936 70.412 71.332z"
                                    transform="translate(-156.205 -52.65)"
                                    fill="url(#ae45afba-725c-45f7-9e29-49ac6fec796e)"
                                ></path>
                                <ellipse
                                    cx="154.941"
                                    cy="527.695"
                                    rx="21.287"
                                    ry="21.566"
                                    fill="url(#f4c06303-04ff-41d3-ad1f-a3b84d982f8e)"
                                ></ellipse>
                                <path
                                    d="M374.423 584.927c0 34.947-63.277 133.913-63.277 133.913s-63.277-98.966-63.277-133.913a63.277 63.277 0 1 1 126.554 0z"
                                    transform="translate(-156.205 -52.65)"
                                    fill="#3f51b5"
                                ></path>
                                <circle
                                    cx="154.941"
                                    cy="527.862"
                                    r="19.13"
                                    fill="#fff"
                                ></circle>
                                <path
                                    d="M717.423 589.927c0 34.947-63.277 133.913-63.277 133.913s-63.277-98.966-63.277-133.913a63.277 63.277 0 1 1 126.554 0z"
                                    transform="translate(-156.205 -52.65)"
                                    fill="#3f51b5"
                                ></path>
                                <circle
                                    cx="497.941"
                                    cy="532.862"
                                    r="19.13"
                                    fill="#fff"
                                ></circle>
                                <path
                                    d="M965.423 549.927c0 34.947-63.277 133.913-63.277 133.913s-63.277-98.966-63.277-133.913a63.277 63.277 0 1 1 126.554 0z"
                                    transform="translate(-156.205 -52.65)"
                                    fill="#3f51b5"
                                ></path>
                                <circle
                                    cx="745.941"
                                    cy="492.862"
                                    r="19.13"
                                    fill="#fff"
                                ></circle>
                                <path
                                    d="M406.423 115.927c0 34.947-63.277 133.913-63.277 133.913s-63.277-98.966-63.277-133.913a63.277 63.277 0 1 1 126.554 0z"
                                    transform="translate(-156.205 -52.65)"
                                    fill="#3f51b5"
                                ></path>
                                <circle
                                    cx="186.941"
                                    cy="58.862"
                                    r="19.13"
                                    fill="#fff"
                                ></circle>
                            </g>
                        </svg>
                    </v-list-item-avatar>
                </v-list-item>
            </v-card>
        </v-col>

        <v-col class="col-12" md="6">
            <pie :height="300"></pie>
        </v-col>

        <v-col class="col-12" md="6">
            <areachart :height="300"></areachart>
        </v-col>

        <v-col class="col-12" md="6">
            <linechart :height="300"></linechart>
        </v-col>

        <v-col class="col-12" md="6">
            <barchart :height="300"></barchart>
        </v-col>
    </v-row>
</template>

<script>
import pie from "./Pie";
import areachart from "./Areachart";
import linechart from "./Linechart";
import barchart from "./Barchart";
export default {
    components: {
        pie,
        areachart,
        linechart,
        barchart
    }
};
</script>
